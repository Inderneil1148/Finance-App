...
import { Settings, Smartphone, Monitor, Plus, CreditCard, Tag, ChartPie } from 'lucide-react';
    ...
    <header className="sticky top-0 z-30 w-full bg-[#F2F2F7]/95 backdrop-blur-xl border-b border-black/[0.06] transition-colors">
      <div className="w-full max-w-7xl mx-auto px-3 sm:px-6 py-2.5 sm:py-3 flex flex-col md:flex-row items-center justify-between gap-2.5 md:gap-4">
        ...
        <div className="w-full md:w-auto flex items-center justify-between gap-3">
          ...
          {/* Desktop Navigation Tabs (Visible on Tablet/Desktop when not in mobile simulator) */}
        ...
        <div className="w-full md:w-auto flex items-center justify-between md:justify-end gap-2 sm:gap-3">
          ...
          {/* Desktop Right Actions: + Add Button, Simulator Toggle, Settings */}
          <div className="hidden md:flex items-center gap-2">
            ...
            <button
              ...
              className={`flex items-center gap-1 px-3 py-1.5 text-xs font-medium rounded-full transition-all min-h-[32px] cursor-pointer active:scale-95 ${
                isMobileSimulator
                  ? 'bg-[#1D1D1F] text-white shadow-xs'
                  : 'text-[#8E8E93] hover:text-[#1D1D1F] bg-black/[0.04] hover:bg-black/[0.08]'
              }`}
              title={isMobileSimulator ? 'Switch to Full Desktop View' : 'Simulate Mobile Phone Frame'}
            >
              {isMobileSimulator ? <Monitor size={14} /> : <Smartphone size={14} />}
              <span className="hidden lg:inline">{isMobileSimulator ? 'Desktop' : 'Mobile View'}</span>
            </button>
